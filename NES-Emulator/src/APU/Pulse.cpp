...
void APU::Pulse::sweepClock() {
	...
	/*if (sweep.reloadFlag)
	{
		if (sweep.enabledFlag && sweep.divider.clock())
		{
			if (sweep.shiftCount != 0 && initialTimer >= 8 && sweep.targetPeriod <= 0x7FF)
			{
				initialTimer = sweep.targetPeriod;
			}
		}
	...
	else
	{
		...
		else if (sweep.enabledFlag && sweep.divider.clock())
		{
			if (sweep.shiftCount != 0 && initialTimer >= 8 && sweep.targetPeriod <= 0x7FF)
			{
				initialTimer = sweep.targetPeriod;
	...
	//if (sweep.divider.counter == 0 && sweep.enabledFlag && sweep.shiftCount != 0)
	//{
	//	// Sweep's muting conditions
	//	if (initialTimer >= 8 && sweep.targetPeriod <= 0x7FF) // !isMuting
	//	{
	//		initialTimer = sweep.targetPeriod;
	//	}
	//	if (sweep.reloadFlag || sweep.divider.counter == 0)
	//	{
	//		sweep.divider.counter = sweep.divider.period;
	//		sweep.reloadFlag = false;
	//	}
